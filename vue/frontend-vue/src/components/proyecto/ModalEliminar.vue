    ...
    <div class="modal-contenido-confirmacion" :class="{ 'theme-dark': isDark, 'theme-light': !isDark }">
      ...
      <div class="modal-body">
        <p class="warning-text">
          ...
          ¿Estás seguro de que deseas eliminar este proyecto? Esta acción no tiene modo de revertirse.
        </p>
        
        <p class="small-info">Proyecto ID: {{ proyectoId }}</p>
      </div>
      
      <div class="pie-modal">
        <button class="btn-cancelar-accion" @click="$emit('cancelar')">Cancelar</button>
        <button class="btn-confirmar-accion" @click="$emit('confirmar')">Sí, Eliminar</button>
      </div>
    </div>
  </div>
</template>
...
<script>
// Asegúrate de importar Font Awesome o Bootstrap Icons en tu main.js
export default {
  name: 'ModalEliminarProyecto',
  props: {
    // Necesario para pasar el ID a la función eliminar del padre
    proyectoId: {
      type: [Number, String],
      required: true
    },
    // El usuario ID se usa en el método 'eliminar' en MisProyectos.vue
    usuarioId: {
      type: [Number, String],
      required: true
    }
  ...
  mounted() {
    // Detección de tema (copia de otros componentes)
