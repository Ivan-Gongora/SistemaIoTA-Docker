<template>
  <div class="grafico-combinado-container" :class="{ 'theme-dark': isDark }">
    <div v-if="loading && !hasData" class="loading-overlay">
      ...
      <p>Procesando niveles de humedad...</p>
    </div>
    <div v-else-if="error" class="error-message">
      <i class="bi bi-exclamation-triangle-fill"></i> {{ error }}
    </div>
    <div v-else-if="!hasData && !loading" class="no-data-message">
      <i class="bi bi-info-circle-fill"></i> Sin registros de humedad.
    </div>
...
const props = defineProps({
  ...
  limites: { type: Object, default: () => ({ humMin: 30, humMax: 65 }) },
    ...
    results.forEach((valores, index) => {
      ...
      if (Array.isArray(valores) && valores.length > 0) {
        ...
        allSeries.push({
          ...
          markLine: { 
            data: [
               { yAxis: props.limites.humMax, name: 'Max', lineStyle: { color: '#3498DB', type: 'dashed' } }
            ], 
            symbol: 'none' 
          }
        });
...
const updateChartOptions = (series) => {
  chartOption.value = {
    title: { text: 'Niveles de Humedad', left: 'center', textStyle: { color: textColor.value } },
    ...
    yAxis: { type: 'value', axisLabel: { color: textColor.value, formatter: '{value}%' }, splitLine: { lineStyle: { color: gridColor.value } }, scale: true },
