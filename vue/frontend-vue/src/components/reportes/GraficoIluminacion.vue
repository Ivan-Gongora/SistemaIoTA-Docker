<template>
  <div class="grafico-combinado-container" :class="{ 'theme-dark': isDark }">
    <div v-if="loading && !hasData" class="loading-overlay">
      ...
      <p>Graficando ciclos de luz...</p>
    </div>
    <div v-else-if="error" class="error-message">
      <i class="bi bi-exclamation-triangle-fill"></i> {{ error }}
    </div>
    <div v-else-if="!hasData && !loading" class="no-data-message">
      <i class="bi bi-info-circle-fill"></i> Sin registros de iluminación (Lux).
    </div>
...
const updateChartOptions = (series, yAxis) => {
  chartOption.value = {
    title: { text: 'Ciclo de Iluminación', left: 'center', textStyle: { color: textColor.value } },
...
.spinner { border: 4px solid rgba(0, 0, 0, 0.1); border-left-color: #f59e0b; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin-bottom: 10px; }
@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
.error-message i, .no-data-message i { margin-right: 8px; font-size: 1.3em; }
.error-message { color: #E74C3C; }
.no-data-message { color: #3498DB; }
.updating-badge { position: absolute; top: 10px; right: 10px; background: rgba(245, 158, 11, 0.9); color: white; padding: 4px 10px; border-radius: 8px; font-size: 0.75rem; font-weight: 800; z-index: 5; }
</style>