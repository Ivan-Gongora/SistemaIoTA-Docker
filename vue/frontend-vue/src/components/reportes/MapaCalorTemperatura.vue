<template>
  <div class="grafico-combinado-container" :class="{ 'theme-dark': isDark }">
    <div v-if="loading && !hasData" class="loading-overlay">
      ...
      <p>Construyendo matriz térmica...</p>
    </div>
    <div v-else-if="error" class="error-message">
      <i class="bi bi-exclamation-triangle-fill"></i> {{ error }}
    </div>
    <div v-else-if="!hasData && !loading" class="no-data-message">
      <i class="bi bi-info-circle-fill"></i> Sin registros térmicos suficientes.
    </div>
...
const updateChartOptions = (dates, hours, data) => {
  ...
  chartOption.value = {
    title: { text: 'Matriz de Calor: Temperatura', left: 'center', textStyle: { color: textColor.value } },
    tooltip: { position: 'top', formatter: (params) => `${dates[params.value[0]]} ${hours[params.value[1]]}<br/><b>${params.value[2]} °C</b>`, backgroundColor: tooltipBgColor.value, textStyle: { color: textColor.value } },
    ...
    visualMap: {
        ...
        inRange: { color: ['#3498DB', '#1ABC9C', '#F1C40F', '#E67E22', '#E74C3C', '#900C3F'] } 
    },
    series: [{
        name: 'Temperatura',
        type: 'heatmap',
