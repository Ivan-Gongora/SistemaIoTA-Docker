  ...
  <div class="plataforma-layout" :class="{ 'theme-dark': isDark, 'theme-light': !isDark }">
    ...
    <div class="plataforma-contenido" :class="{ 'shifted': isSidebarOpen }">
      ...
      <div class="dashboard-grid-premium">
        ...
        <template v-if="tabActiva === 'clima' && resultado && !loading">
            ...
            <div class="chart-container-v2 shadow-lg">
              <div class="chart-header-v2 mb-4 d-flex justify-content-between align-items-start">
                <div>
                  ...
                  <span class="text-muted-contrast small">Nivel de influencia de las variables climatológicas sobre el gasto energético.</span>
                </div>
              </div>
            ...
            <div class="chart-container-v2 shadow-lg">
              <div class="chart-header-v2 mb-4 d-flex justify-content-between align-items-start">
                <div>
                  ...
                  <span class="text-muted-contrast small">Variación del gasto eléctrico al cambiar la temperatura exterior.</span>
                </div>
              </div>
          ...
          <div class="grid-item-header">
            <div class="chart-container-v2 shadow-lg">
              <div class="chart-header-v2 mb-4 d-flex justify-content-between align-items-start">
                <div>
                  <h5 class="fw-bold m-0 title-contrast">Climatología Ambiental Histórica</h5>
            ...
            <div class="chart-container-v2 shadow-lg">
              <div class="chart-header-v2 mb-4 d-flex justify-content-between align-items-start">
                <div>
                  ...
                  <span class="text-muted-contrast small">Distribución de los costos facturados en cada periodo tarifario.</span>
                </div>
            ...
            <div class="chart-container-v2 shadow-lg">
              <div class="chart-header-v2 mb-4 d-flex justify-content-between align-items-start">
                <div>
                  ...
                  <span class="text-muted-contrast small">Consumo útil contrastado con la energía desperdiciada en horarios inactivos.</span>
                </div>
        ...
        <div class="grid-item-data-tables" v-if="resultado && !loading">
          <div ref="tablaDiaria" class="glass-card shadow-lg p-0 overflow-hidden">
            <div class="table-header-v2 p-4 d-flex justify-content-between align-items-center flex-wrap gap-3">
              <div>
                ...
                <p class="text-muted-contrast small mb-0">Cruce extendido entre consumo eléctrico y lecturas ambientales desplegado de forma horizontal.</p>
              </div>
...
export default {
  ...
  methods: {
    ...
    renderClima() {
      if (!this.$refs.chartClima) return;
      const textColor = this.isDark ? '#f8fafc' : '#1e293b';
      const gridColor = this.isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)';
      const tooltipBg = this.isDark ? '#1a1d2d' : '#ffffff';
      ...
      this.instances.clima.setOption({
        ...
        yAxis: [
          ...
          { type: 'value', name: '%', axisLabel: { color: textColor }, splitLine: { lineStyle: { color: gridColor, type: 'dashed' } }, scale: true }
        ],
        series: [
          { 
            name: 'Temperatura', 
    ...
    renderRadar() {
      if (!this.$refs.chartRadar) return;
      const textColor = this.isDark ? '#f8fafc' : '#1e293b';
      const gridColor = this.isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)';
      const tooltipBg = this.isDark ? '#1a1d2d' : '#ffffff';
      ...
      const labels = ['Temp Int', 'Humedad Int', 'Iluminación', 'Temp Ext', 'Humedad Ext'];
      const mesData = [
      ...
      this.instances.radar.setOption({
        color: ['#3b82f6', '#10b981'],
        tooltip: { trigger: 'axis', backgroundColor: tooltipBg, borderColor: gridColor, textStyle: { color: textColor }, axisPointer: { type: 'shadow' } },
        ...
        grid: { top: 30, left: 90, right: 40, bottom: 50, containLabel: true },
        xAxis: { type: 'value', min: -1, max: 1, splitLine: { lineStyle: { color: gridColor } }, axisLabel: { color: textColor } },
        yAxis: { type: 'category', data: labels, axisLabel: { color: textColor, fontWeight: 'bold' } },
        series: [
          { name: 'Promedio Mensual', type: 'bar', data: mesData },
          { name: 'Día Típico', type: 'bar', data: diaData }
        ]
      }, true);
    },

    renderScatter() {
      if (!this.$refs.chartScatter) return;
      const textColor = this.isDark ? '#f8fafc' : '#1e293b';
      const gridColor = this.isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.05)';
      const tooltipBg = this.isDark ? '#1a1d2d' : '#ffffff';
      ...
      this.instances.scatter.setOption({
        ...
        xAxis: { type: 'value', name: 'Temperatura (°C)', nameLocation: 'middle', nameGap: 30, scale: true, axisLabel: { color: textColor }, splitLine: { lineStyle: { color: gridColor } } },
        yAxis: { type: 'value', name: 'Consumo (kWh)', scale: true, axisLabel: { color: textColor }, splitLine: { lineStyle: { color: gridColor } } },
        series: [
...
@media (max-width: 1200px) {
  .grid-item-header, .grid-item-alert, .grid-item-kpis, .grid-item-data-tables, .grid-item-footer, .grid-item-tabs { grid-column: 1 / -1; }
  .grid-item-chart-main, .grid-item-chart-side, .grid-item-chart-half { grid-column: 1 / -1; }
