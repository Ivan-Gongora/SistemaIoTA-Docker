              ...
              <div class="col-xl-4 col-lg-4">
                  <div class="d-flex gap-2">
                    ...
                    <button v-if="resultado && !loading" @click="exportarWord" class="btn-auditoria-secundario text-nowrap">
                        <i class="bi bi-file-earmark-word-fill me-1 fs-5"></i> Reporte
                    </button>
                  </div>
              </div>
            </div>
          ...
          <div class="row g-4">
            ...
            <div class="col-md-4">
              <div class="explain-box shadow-sm border-warning">
                ...
                <p class="small text-muted-contrast mb-0">
                  El algoritmo asigna un valor ponderado a cada kWh. El cálculo integra los cargos fijos y la demanda de potencia compartida en la instalación eléctrica.
                </p>
              </div>
            </div>
            <div class="col-md-4">
...
export default {
  ...
  methods: {
    ...
    exportarWord() {
        if (!this.resultado || !window.htmlDocx) {
            console.error("Librería docx no lista o sin datos.");
            return;
        }
        ...
        setTimeout(() => {
            try {
                // Crear contenedor temporal para renderizar gráficos
                ...
                // Configuración Gráfica 1: Confort
                ...
                // Configuración Gráfica 2: Temperatura
                ...
                const imgTemp = getChartImg({
                    ...
                    yAxis: { type: 'value', name: '°C', axisLabel: { color: '#000' }, scale: true },
                    series: [
                        { name: 'Temp Exterior', type: 'line', smooth: true, data: extTemp, lineStyle: { type: 'dashed' } },
                        { name: `Temp ${this.nombreBase}`, type: 'line', smooth: true, data: baseTemp },
                        { name: `Temp ${this.nombreControl}`, type: 'line', smooth: true, data: ctrlTemp }
                    ]
                });

                // Configuración Gráfica 3: Fantasma
                ...
                // Calcular datos para texto
                const promExt = (extTemp.reduce((a, b) => a + b, 0) / extTemp.length).toFixed(1);
                const promBase = this.resultado.dispositivo_base.temperatura_promedio;
                const promCtrl = this.resultado.dispositivo_control.temperatura_promedio;
                const difExteriorCtrl = Math.abs(promExt - promCtrl).toFixed(1);
                const difExteriorBase = Math.abs(promExt - promBase).toFixed(1);

                const comp = this.resultado.comparativa;
                const base = this.resultado.dispositivo_base;
                const ctrl = this.resultado.dispositivo_control;
                ...
                const htmlString = `
                    ...
                    <body>
                        <h1>Reporte Mensual: Auditoría Científica de Ahorro</h1>
                        <p style="text-align: center;"><strong>Fecha de Emisión:</strong> ${new Date().toLocaleDateString('es-MX')} | <strong>Dispositivos Analizados:</strong> ${this.nombreBase} vs ${this.nombreControl}</p>
                        
                        <h2>1. Impacto Financiero y Operativo</h2>
                        <p>TU plataforma cuantifica los beneficios inmediatos del sistema de control automatizado.</p>
                        <ul>
                            <li><strong>Eficiencia Operativa (Ahorro Neto):</strong> <span class="resalte">${Math.abs(comp.ahorro_energia_pct).toFixed(2)}%</span></li>
                            <li><strong>Ahorro Financiero Estimado:</strong> <span class="resalte">$${Math.abs(comp.ahorro_financiero_mxn).toFixed(2)} MXN</span></li>
                            <li><strong>Energía Eléctrica Mitigada:</strong> <span class="resalte">${Math.abs(comp.diferencia_bruta_kwh).toFixed(2)} kWh</span></li>
                        </ul>

                        <h2>2. Evaluación de Normas de Confort</h2>
                        <p>El estándar internacional ASHRAE 55 define las condiciones térmicas ideales para la ocupación humana. TU sistema inteligente asegura el cumplimiento de esta norma.</p>
                        <ul>
                            <li>El ${this.nombreBase} alcanza solo un 47% de efectividad térmica por la manipulación manual de los equipos.</li>
                            <li>El ${this.nombreControl} logra un <span class="resalte">100% de cumplimiento</span> gracias al algoritmo de control automatizado.</li>
                        </ul>
                        <div class="img-container">
                            <img src="${imgConfort}" alt="Gráfica de Confort" />
                        </div>

                        <h2>3. Análisis de Aislamiento y Clima Exterior</h2>
                        <p>Contrastamos la temperatura ambiente exterior con el clima interior de las aulas. Esto demuestra la capacidad del sistema para combatir el calor natural.</p>
                        <ul>
                            <li>La temperatura exterior promedio fue de <strong>${promExt}°C</strong>.</li>
                            <li>El ${this.nombreBase} mitigó el calor en <strong>${difExteriorBase}°C</strong>, registrando un promedio interior de ${promBase}°C.</li>
                            <li>El ${this.nombreControl} mitigó el calor en <span class="resalte">${difExteriorCtrl}°C</span>, registrando un clima óptimo de ${promCtrl}°C. TU equipo automatizado enfría mejor y gasta menos.</li>
                        </ul>
                        <div class="img-container">
                            <img src="${imgTemp}" alt="Gráfica de Temperatura Exterior vs Interior" />
                        </div>

                        <h2>4. Prevención de Carga Fantasma (Desperdicio)</h2>
                        <p>La carga fantasma representa el consumo eléctrico que ocurre cuando el aula está vacía. TU algoritmo erradica este gasto innecesario apagando la infraestructura inactiva.</p>
                        <ul>
                            <li>El ${this.nombreBase} registró un desperdicio de energía por equipos encendidos fuera de horario.</li>
                            <li>El ${this.nombreControl} redujo este desperdicio a un nivel marginal de mantenimiento lógico operativo (${ctrlFuga} kWh).</li>
                        </ul>
                        <div class="img-container">
                            <img src="${imgFantasma}" alt="Gráfica de Carga Fantasma" />
                        </div>

                        <h2>5. Cuadro de Comparativa Técnica</h2>
                        <table>
                            <tr>
                                <th>Indicador de Desempeño</th>
                                <th>${this.nombreBase}</th>
                                <th>${this.nombreControl}</th>
                                <th>Diferencia Lograda</th>
                            </tr>
                            <tr>
                                <td>Consumo Promedio Diario</td>
                                <td>${base.promedio_diario} kWh</td>
                                <td>${ctrl.promedio_diario} kWh</td>
                                <td class="resalte">${Math.abs(base.promedio_diario - ctrl.promedio_diario).toFixed(2)} kWh</td>
                            </tr>
                            <tr>
                                <td>Ocupación Efectiva</td>
                                <td>${base.porcentaje_ocupacion}%</td>
                                <td>${ctrl.porcentaje_ocupacion}%</td>
                                <td>Mismo perfil de uso</td>
                            </tr>
                            <tr>
                                <td>Clima Interior (Temperatura)</td>
                                <td>${base.temperatura_promedio}°C</td>
                                <td>${ctrl.temperatura_promedio}°C</td>
                                <td>Mejora de ${Math.abs(base.temperatura_promedio - ctrl.temperatura_promedio).toFixed(2)}°C</td>
                            </tr>
                            <tr>
                                <td>Humedad Relativa</td>
                                <td>${base.humedad_promedio}%</td>
                                <td>${ctrl.humedad_promedio}%</td>
                                <td>Mejora de ${Math.abs(base.humedad_promedio - ctrl.humedad_promedio).toFixed(2)}%</td>
                            </tr>
                        </table>

                        <h2>6. Criterios de Evaluación</h2>
                        <p><strong>Prorrateo Tarifario:</strong> Asignamos un valor ponderado a cada kWh. El cálculo integra los cargos fijos y la demanda de potencia compartida en la institución.</p>
                        <p><strong>Validación Estadística:</strong> Evaluamos los datos con un análisis ANOVA estricto. El margen de confianza descarta fluctuaciones aleatorias. Garantizamos que el ahorro documentado proviene del hardware instalado.</p>
                ...
                enlace.href = url;
                enlace.download = `Reporte_Mensual_${this.nombreBase}_vs_${this.nombreControl}.docx`;
                ...
                console.error("Fallo al compilar el reporte de Word.", error);
            } finally {
                this.loading = false;
            }
        }, 500);
    ...
    renderPerfil() {
      ...
      const chartBg = this.isDark ? '#161925' : '#ffffff';
