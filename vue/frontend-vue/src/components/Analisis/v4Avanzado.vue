              ...
              <table class="table-auditoria">
                <thead>
                  <tr>
                    ...
                    <th class="text-center text-success">AHORRO OBTENIDO</th>
                ...
                <tbody>
                  <tr v-for="(item, index) in resultado.dispositivo_base.grafica_tendencia_diaria" :key="index">
                    ...
                    <td class="text-center text-success fw-bold bg-success bg-opacity-10">
...
export default {
  ...
  methods: {
    ...
    exportarCSV() {
      ...
      const BOM = "\uFEFF";
      let csvContent = BOM + `FECHA,${this.nombreBase} (kWh),${this.nombreControl} (kWh),AHORRO OBTENIDO (kWh),TEMP MEDIA (°C),HUMEDAD (%)\n`;

      this.resultado.dispositivo_base.grafica_tendencia_diaria.forEach((item, index) => {
        const ctrlItem = this.resultado.dispositivo_control.grafica_tendencia_diaria[index];
        ...
        const fila = `${item.fecha},${item.kwh.toFixed(2)},${ctrlItem.kwh.toFixed(2)},${ahorro},${temp},${hum}`;
        csvContent += fila + "\n";
      });
      ...
      link.setAttribute("download", "registro_tecnico_diario.csv");
...
.table-auditoria { 
  ...
  th { padding: 20px; font-size: 0.75rem; font-weight: 900; color: $ACCENT-BLUE; letter-spacing: 1.5px; position: sticky; top: 0; z-index: 10; } 
