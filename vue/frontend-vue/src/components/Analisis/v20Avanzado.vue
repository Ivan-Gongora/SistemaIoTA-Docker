...
export default {
  ...
  methods: {
    ...
    async exportarTablaLarga(refName, fileName) {
      ...
      try {
        const elemento = this.$refs[refName];
        const contenedorScroll = elemento.querySelector('.table-responsive');
        
        // Guardar dimensiones originales
        const originalOverflow = contenedorScroll.style.overflow;
        const originalMaxWidth = contenedorScroll.style.maxWidth;
        const originalWidthScroll = contenedorScroll.style.width;
        
        const originalPosition = elemento.style.position;
        const originalWidthElem = elemento.style.width;
        const originalZIndex = elemento.style.zIndex;

        // Forzar expansión total sin alterar el flujo visual permanentemente
        contenedorScroll.style.overflow = 'visible';
        contenedorScroll.style.maxWidth = 'none';
        contenedorScroll.style.width = 'max-content';
        
        elemento.style.position = 'absolute';
        elemento.style.width = 'max-content';
        elemento.style.zIndex = '9999';

        // Dar tiempo al navegador para recalcular el diseño antes de capturar
        ...
        contenedorScroll.style.overflow = originalOverflow;
        contenedorScroll.style.maxWidth = originalMaxWidth;
        contenedorScroll.style.width = originalWidthScroll;
        
        elemento.style.position = originalPosition;
        elemento.style.width = originalWidthElem;
